                        ...
                        if codec_song in LEGACY_CODECS:
                            ...
                            if not stream_info.stream_url or not stream_info.pssh:
                                ...
                                if not SongCodec.AAC_LEGACY == codec_song:
                                    main(urls=[track["attributes"]['url']],language=language, output_path=output_path, codec_song=SongCodec.AAC_LEGACY, download_mode=download_mode, overwrite=overwrite, no_synced_lyrics=no_synced_lyrics,nm3u8dlre_path=nm3u8dlre_path,mp4decrypt_path=mp4box_path,ffmpeg_path=ffmpeg_path,window=window)
                                continue
